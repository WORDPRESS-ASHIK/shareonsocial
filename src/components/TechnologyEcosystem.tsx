...
export function TechnologyEcosystem() {
  return (
    <div className="relative w-full mt-8 pt-4 pb-8 flex flex-col items-center justify-center">
      {/* Background glow */}
      <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[60%] h-[100px] bg-[color:var(--color-brand-cyan)]/10 blur-[40px] rounded-full pointer-events-none z-0"></div>
          ...
          <React.Fragment key={node.id}>
            ...
            >
              <div
                className={`relative flex flex-col items-center justify-center p-1.5 md:p-2 rounded-xl backdrop-blur-md border shadow-lg transition-all duration-300 group-hover:scale-105 group-hover:-translate-y-1 
                  ${
                    node.isAi
                      ? "bg-[color:var(--color-brand-cyan)]/20 border-[color:var(--color-brand-cyan)]/50 shadow-[0_0_15px_rgba(34,211,238,0.4)]"
                      : "bg-white/10 border-white/20 group-hover:bg-white/20"
                  }
                `}
              >
                <node.icon className={`w-3.5 h-3.5 md:w-4 md:h-4 ${node.isAi ? "text-[color:var(--color-brand-cyan)]" : "text-white"}`} />
                {node.isAi && (
              ...
              {/* Connection Dot Glow */}
              <div className="absolute top-1/2 -right-[3px] md:-right-[4px] -translate-y-1/2 w-1 h-1 md:w-1.5 md:h-1.5 rounded-full bg-[color:var(--color-brand-cyan)] opacity-50 shadow-[0_0_5px_#22d3ee]"></div>
              <div className="absolute top-1/2 -left-[3px] md:-left-[4px] -translate-y-1/2 w-1 h-1 md:w-1.5 md:h-1.5 rounded-full bg-[color:var(--color-brand-cyan)] opacity-50 shadow-[0_0_5px_#22d3ee]"></div>
            </motion.div>
            ...
            {i < nodes.length - 1 && (
              <div className="flex-1 h-[1px] bg-white/10 relative z-0 min-w-[4px] md:min-w-[8px] mx-0.5 md:mx-1 overflow-hidden">
                <motion.div
                  className="absolute top-0 left-0 h-full w-[15px] bg-gradient-to-r from-transparent via-[color:var(--color-brand-cyan)] to-transparent shadow-[0_0_8px_#22d3ee]"
                  animate={{ left: ["-20%", "120%"] }}
                  transition={{
                    duration: 1.5,
                    repeat: Infinity,
